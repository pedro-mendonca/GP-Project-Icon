...
$breadcrumbs = array(
	...
	'/tools/glossaries/' => esc_html__( 'Glossaries', 'gp-project-icon' ),
);
...
$args = array(
	'title'       => esc_html__( 'Glossaries', 'gp-project-icon' ), // Page title.
	'description' => esc_html__( 'Overview of all Global and Project Glossaries.', 'gp-project-icon' ), // Page description.
);
...
<p class="gptoolbox-description">
	<?php echo wp_kses_post( __( 'The type of Glossaries can be <code>global</code> or <code>project</code>.', 'gp-project-icon' ) ); ?>
	<br>
	<?php echo wp_kses_post( __( 'Each Glossary belongs to a Translation Set, identified by <code>translation_set_id</code>. If there is no Translation Set in the database with the same ID, then the Glossary is orphaned.', 'gp-project-icon' ) ); ?>
</p>
...
?>
<section class="gp-project-icon glossaries">
	<?php

	// Check for glossaries.
	if ( empty( $gp_glossaries ) ) {
		?>
		<p id="glossaries-type-filters"><?php esc_html_e( 'No glossaries found.', 'gp-project-icon' ); ?></p>
		<?php
	} else {
		?>
		<p id="glossaries-type-filters">
			...
			echo wp_kses_post(
				sprintf(
					...
					__( 'Glossaries: %s', 'gp-project-icon' ),
					sprintf(
						'%1$s %2$s %3$s %4$s %5$s',
						'<a id="glossaries-type-all" class="glossaries-type" href="#glossaries-type-filters">' . sprintf(
							/* translators: %s: Number of Glossaries. */
							_n( '%s glossary.', 'All %s glossaries.', $global_glossaries_count + $project_glossaries_count + $unknown_type_glossaries_count + count( $orphaned_glossary_entries ), 'gp-project-icon' ),
						...
						$global_glossaries_count > 0 ? '<a id="glossaries-type-global" class="glossaries-type" href="#glossaries-type-filters">' . sprintf(
							/* translators: %s: Number of Glossaries. */
							_n( '%s Global glossary.', '%s Global glossaries.', $global_glossaries_count, 'gp-project-icon' ),
							'<strong class="glossaries-label glossaries-label-global">' . esc_html( number_format_i18n( $global_glossaries_count ) ) . '</strong>'
						) . '</a>' : '',
						$project_glossaries_count > 0 ? '<a id="glossaries-type-project" class="glossaries-type" href="#glossaries-type-filters">' . sprintf(
							/* translators: %s: Number of Glossaries. */
							_n( '%s Project glossary.', '%s Project glossaries.', $project_glossaries_count, 'gp-project-icon' ),
							'<strong class="glossaries-label glossaries-label-project">' . esc_html( number_format_i18n( $project_glossaries_count ) ) . '</strong>'
						) . '</a>' : '',
						$unknown_type_glossaries_count > 0 ? '<a id="glossaries-set-unknown" class="glossaries-type" href="#glossaries-type-filters">' . sprintf(
							/* translators: %s: Number of Glossaries. */
							_n( '%s Glossary of unknown translation set.', '%s Glossaries of unknown translation set.', $unknown_type_glossaries_count, 'gp-project-icon' ),
							'<strong class="glossaries-label glossaries-label-project">' . esc_html( number_format_i18n( $unknown_type_glossaries_count ) ) . '</strong>'
						) . '</a>' : '',
						count( $orphaned_glossary_entries ) > 0 ? '<a id="glossaries-unknown-orphaned-entries" class="glossaries-type" href="#glossaries-type-filters">' . sprintf(
							/* translators: %s: Number of Glossaries. */
							_n( 'Entries from %s unknown glossary.', 'Entries from %s unknown glossaries.', count( $orphaned_glossary_entries ), 'gp-project-icon' ),
	...
	if ( ! empty( $gp_glossaries ) || ! empty( $orphaned_glossary_entries ) ) {
		...
		<div class="glossaries-filter">
			<label for="glossaries-filter"><?php esc_html_e( 'Filter:', 'gp-project-icon' ); ?> <input id="glossaries-filter" type="text" placeholder="<?php esc_attr_e( 'Search', 'gp-project-icon' ); ?>" /> </label>
			<button id="glossaries-filter-clear" class="button" style="margin-bottom: 3px;" title="<?php esc_attr_e( 'Clear search filter.', 'gp-project-icon' ); ?>"><?php esc_html_e( 'Clear', 'gp-project-icon' ); ?></button>
		</div>

		<table class="gp-table gp-project-icon tools-glossaries">
			<thead>
				<tr>
					<th class="gp-column-id"><?php esc_html_e( 'ID', 'gp-project-icon' ); ?></th>
					<th class="gp-column-type"><?php esc_html_e( 'Type', 'gp-project-icon' ); ?></th>
					<th class="gp-column-locale"><?php esc_html_e( 'Locale', 'gp-project-icon' ); ?></th>
					<th class="gp-column-translation-set"><?php esc_html_e( 'Project', 'gp-project-icon' ); ?></th>
					<th class="gp-column-entries"><?php esc_html_e( 'Entries', 'gp-project-icon' ); ?></th>
					...
					<tr gptoolboxdata-glossary="<?php echo esc_attr( strval( $glossary->id ) ); ?>">
						...
						if ( ! $translation_set ) {
							...
							<td class="type global" data-text="global">
								<?php
								// Global glossary.
								esc_html_e( 'Global', 'gp-project-icon' );
								?>
							</td>
							...
							<td class="type project" data-text="project">
								<?php
								// Project glossary.
								esc_html_e( 'Project', 'gp-project-icon' );
								?>
							</td>
							<?php
