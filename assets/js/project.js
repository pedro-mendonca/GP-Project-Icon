...
jQuery( document ).ready( function( $ ) {
	...
	if ( tableTranslationSets.length ) {
		...
		if ( supportedTranslationStatuses.hasOwnProperty( 'old' ) ) {
			...
			$( tableTranslationSets ).children( 'thead' ).children( 'tr' ).find( 'th:last' ).after( '<th class="gptoolbox-column-old">' + wp.i18n.__( 'Old', 'gp-project-icon' ) + '</td>' );
		}

		// Check if the 'rejected' status is supported.
		if ( supportedTranslationStatuses.hasOwnProperty( 'rejected' ) ) {
			// Add column header to header row.
			$( tableTranslationSets ).children( 'thead' ).children( 'tr' ).find( 'th:last' ).after( '<th class="gptoolbox-column-rejected">' + wp.i18n.__( 'Rejected', 'gp-project-icon' ) + '</td>' );
		}

		// Check if the 'changesrequested' status is supported.
		if ( supportedTranslationStatuses.hasOwnProperty( 'changesrequested' ) ) {
			// Add column header to header row.
			$( tableTranslationSets ).children( 'thead' ).children( 'tr' ).find( 'th:last' ).after( '<th class="gptoolbox-column-changesrequested">' + wp.i18n.__( 'Changes requested', 'gp-project-icon' ) + '</td>' );
		}

		// Add 'warnings' column header to header row.
		$( tableTranslationSets ).children( 'thead' ).children( 'tr' ).find( 'th:last' ).after( '<th class="gptoolbox-column-warnings">' + wp.i18n.__( 'Warnings', 'gp-project-icon' ) + '</td>' );
	...
	function translationsBulkDelete( locale, slug, status ) {
		...
		$.ajax( {
			...
			url: wpApiSettings.root + 'gp-project-icon/v1/translations/' + project.path + '/' + locale + '/' + slug + '/' + status + '/-delete',
			type: 'POST',
			data: {
